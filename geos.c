...
void c_GEOSGeomBounds_r(GEOSContextHandle_t handle, const GEOSGeometry *g, double *minX, double *minY, double *maxX,
  ...
  switch (GEOSGeomTypeId_r(handle, g)) {
    ...
    case GEOS_POLYGON:
      c_GEOSGeomBounds_r(handle, GEOSGetExteriorRing_r(handle, g), minX, minY, maxX, maxY);
      for (int i = 0, n = GEOSGetNumInteriorRings_r(handle, g); i < n; ++i) {
        c_GEOSGeomBounds_r(handle, GEOSGetInteriorRingN_r(handle, g, i), minX, minY, maxX, maxY);
      }
      break;
    ...
    case GEOS_GEOMETRYCOLLECTION:
      for (int i = 0, n = GEOSGetNumGeometries_r(handle, g); i < n; ++i) {
        c_GEOSGeomBounds_r(handle, GEOSGetGeometryN_r(handle, g, i), minX, minY, maxX, maxY);
      }
      break;
  }
}
...
GEOSGeometry *c_newGEOSGeomFromBounds_r(GEOSContextHandle_t handle, int *typeID, double minX, double minY, double maxX,
  ...
  if (minX == maxX && minY == maxY) {
    ...
    if (GEOSCoordSeq_setX_r(handle, s, 0, minX) == 0 || GEOSCoordSeq_setY_r(handle, s, 0, minY) == 0) {
      GEOSCoordSeq_destroy_r(handle, s);
      return NULL;
    }
  ...
  const double flatCoords[10] = {minX, minY, maxX, minY, maxX, maxY, minX, maxY, minX, minY};
  const double *val           = flatCoords;
  for (unsigned idx = 0; idx < 5; idx++) {
    if (GEOSCoordSeq_setX_r(handle, s, idx, *val++) == 0 || GEOSCoordSeq_setY_r(handle, s, idx, *val++) == 0) {
      GEOSCoordSeq_destroy_r(handle, s);
      return NULL;
    }
  }
