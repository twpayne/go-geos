...
func (c *Context) NewPolygon(coordss [][][]float64) *Geom {
	...
	if nholes > 0 {
		...
		for i := 0; i < nholes; i++ {
			holesSlice[i] = C.GEOSGeom_createLinearRing_r(c.handle, c.newGEOSCoordSeqFromCoords(coordss[i+1]))
			if holesSlice[i] == nil {
				panic(c.err)
			}
		}
...
func (c *Context) newCoordsFromGEOSCoordSeq(s *C.struct_GEOSCoordSeq_t) [][]float64 {
	...
	for i := 0; i < int(size); i++ {
		coords[i] = flatCoords[i*int(dimensions) : (i+1)*int(dimensions) : (i+1)*int(dimensions)]
	}
	return coords
}
...
func (c *Context) newGEOSCoordSeqFromCoords(coords [][]float64) *C.struct_GEOSCoordSeq_t {
	...
	flatCoords := make([]float64, dimensions*len(coords))
	for i, coord := range coords {
		copy(flatCoords[dimensions*i:dimensions*(i+1)], coord)
	}
