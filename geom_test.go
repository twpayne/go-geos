...
func TestGeomMethods(t *testing.T) {
	...
	pts, err := unitSquare.NearestPoints(mustNewGeomFromWKT(t, c, "POLYGON ((2 2, 3 2, 3 3, 2 3, 2 2))"))
	assert.Nil(t, err)
	assert.True(t, pts[0].Equals(mustNewGeomFromWKT(t, c, "POINT (1 1)")) && pts[1].Equals(mustNewGeomFromWKT(t, c, "POINT (2 2)")))
	_, err = unitSquare.NearestPoints(mustNewGeomFromWKT(t, c, "GEOMETRYCOLLECTION EMPTY"))
	assert.NotNil(t, err)
